...
Question 3 (a) : Now calculate the new variable burdenS as the sum of the ranks of the four variables
E070M, E070V, E072, E074 for each person (i.e. sum of the numerical factor levels).
...
Question 3 (a) : Add this variabes burdenS to the dataset kiggs, and save it in its updated form as an RData
file (overwrite the previous file).
...
Calculate absolute frequency tables for age2, sex, burdenS, and mean + standard
deviation for arztZ01. Display them in a table or describe them in continuous text.
...
Also indicate how many missing values each of these 4 variables has, and how
many observations have complete data for these 4 variables.
...
5. a) Calculate a linear regression, with arztZ01 as outcome and the predictors burdenS, sex,
age2, schichtz and e065z.
...
5.c) Since there is evidence that individuals drawn from the same area are correlated with each
other, but we are not interested in the effect of the area on the health ... what would be a
suitable strategy for accounting for this correlation?

One possible strategy is to include area as a random effect in your statistical model.



Question 6 – Sample size calculation

6.a) Look at the literature or think for yourself based on expert knowledge what effect size
you would expect. State the effect size that you are assuming and explain why.
...
6.c) Now compute the minimum necessary sample size for a power of 80% and a significance
threshold of alpha = 0.05, for example by using a function in the R package pwr. What is
the sample size?
```{r}
library(pwr)
